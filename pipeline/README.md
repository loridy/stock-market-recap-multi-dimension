...
| 1b| `fetch-news.mjs`          | `data/YYYY-MM-DD/news.json`          | RSS baseline + optional newsfilter secondary merge (normalized + deduped) |
...
- Current RSS list includes:
  - Thomson Reuters IR News feed
  - CNBC Markets
  - MarketWatch Top Stories
- If `NEWSFILTER_API_KEY` is set, `fetch-news.mjs` also pulls newsfilter results and merges them as a secondary source.
- If `NEWSFILTER_API_KEY` is not set, pipeline stays in RSS-only mode.
